...
const App: React.FC = () => {
  ...
  const [paymentModalReason, setPaymentModalReason] = useState('');
  ...
  const handleRequireCredits = (reason: string) => {
    setPaymentModalReason(reason);
    setIsPaymentModalOpen(true);
  };

  const handleOpenPaymentModal = () => {
    setPaymentModalReason('');
    setIsPaymentModalOpen(true);
  };
  ...
  const handleGarmentSelect = useCallback(async (garmentFile: File, garmentInfo: WardrobeItem) => {
    ...
    if (!spendCredits(1)) {
      handleRequireCredits(`You need 1 credit to add the ${garmentInfo.name}.`);
      return;
    }
  ...
  }, [displayImageUrl, isLoading, currentPoseIndex, outfitHistory, currentOutfitIndex, spendCredits]);
  ...
  const handlePoseSelect = useCallback(async (newIndex: number) => {
    ...
    if (!spendCredits(1)) {
      handleRequireCredits('You need 1 credit to generate a new pose.');
      return;
    }
  ...
  }, [currentPoseIndex, outfitHistory, isLoading, currentOutfitIndex, spendCredits]);
    ...
    <div className="font-sans">
      <AnimatePresence mode="wait">
        {!modelImageUrl ? (
          ...
          >
            <StartScreen onModelFinalized={handleModelFinalized} onRequireCredits={handleRequireCredits}/>
          </motion.div>
        ) : (
          <motion.div
            key="main-app"
            ...
            transition={{ duration: 0.5, ease: 'easeInOut' }}
          >
            <Header onOpenPaymentModal={handleOpenPaymentModal} />
            <main className="flex-grow relative flex flex-col md:flex-row overflow-hidden pt-16">
      ...
      <PaymentModal
        ...
        triggerReason={paymentModalReason}
      />
    </div>
  );
};
